...
Output: docs/ce_open_signals_<timestamp>.html (GitHub Pages)
"""
...
def main():
    ...
    fname    = f"ce_open_signals_{now.strftime('%Y-%m-%d_%I-%M-%p')}.html"
    ...
    all_files = sorted(
        [f for f in os.listdir(out_dir) if f.startswith("ce_open_signals_") and f.endswith(".html")],
        reverse=True
    )
    ...
    def make_index(files, generated_at):
        ...
        for i, f in enumerate(files):
            try:
                ts_part = f.replace("ce_open_signals_", "").replace(".html", "")
